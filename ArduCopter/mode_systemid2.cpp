...
const AP_Param::GroupInfo ModeSystemId2::var_info[] = {
    ...
    AP_GROUPINFO_FLAGS("_AXIS", 1, ModeSystemId2, axis, 0, AP_PARAM_FLAG_ENABLE),

    // @Param: _MAGNITUDE
    // @DisplayName: System identification Chirp Magnitude
    // @Description: Magnitude of sweep in deg, deg/s and 0-1 for mixer outputs.
    // @User: Standard
    AP_GROUPINFO("_MAGNITUDE", 2, ModeSystemId2, waveform_magnitude, 15),
    ...
    // @Units: Hz
    // @User: Standard
    AP_GROUPINFO("_F_START_HZ", 3, ModeSystemId2, frequency_start, 0.5f),
    ...
    // @Units: Hz
    // @User: Standard
    AP_GROUPINFO("_F_STOP_HZ", 4, ModeSystemId2, frequency_stop, 40),
    ...
    // @Units: s
    // @User: Standard
    AP_GROUPINFO("_T_FADE_IN", 5, ModeSystemId2, time_fade_in, 15),
    ...
    // @Units: s
    // @User: Standard
    AP_GROUPINFO("_T_REC", 6, ModeSystemId2, time_record, 70),
    ...
    // @Units: s
    // @User: Standard
    AP_GROUPINFO("_T_FADE_OUT", 7, ModeSystemId2, time_fade_out, 2),
...
bool ModeSystemId2::init(bool ignore_checks)
{
    // check if enabled
    if (axis == 0) {
        gcs().send_text(MAV_SEVERITY_WARNING, "No axis selected, SID2_AXIS = 0");
        return false;
    }
...
void ModeSystemId2::log_data() const
{
